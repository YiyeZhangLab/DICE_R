...
#' }
#' @return Called for its side effects: trains the model and saves results to
...
DICEr <- function(args) {
  ...
  message("/////////////////////////////////////////////////////////////////////////////")
  message("part 1: train AE and for representation initialization")
  ...
  for (epoch in seq_len(args$init_AE_epoch)) {
    ...
    message(sprintf("Epoch: %s, train AE loss: %s, test AE loss: %s.", epoch, train_AE_loss, test_AE_loss))
  ...
  message("part 1, initial done!")
  message("////////////////////////////////////////////////////////////////////////////////////")
  message("part 2, start optimization the main loss")
  ...
  for (iter_i in seq_len(args$iter)) {
    ...
    message(paste("    final_embed.shape=", dim(final_embed)))

    random_state <- sample(1:1234, 1)
    message(paste("random_state=", random_state))
    ...
    message(paste("number_reassign=", number_reassign))
    ...
    message("***************************************")
    message(paste("data_train.M[0,:]=", as.numeric(data_train$M[1,])))
    update_M(data_train)
    message(paste("data_train.M[0,:]=", as.numeric(data_train$M[1,])))

    message(paste("    data_train.M.shape=", dim(data_train$M)))
    message(paste("    data_train.C.shape=", dim(data_train$C)))
    message(paste("    data_train.rep.shape=", dim(data_train$rep)))
    message("4. init train *.M, *.C, *.rep done!")
    ...
    for (epoch in seq_len(args$epoch_in_iter)) {
      ...
      message("---------------------------------------------------------------------------------")
      message(paste("iter_i = ", iter_i, ", epoch=", epoch))
      ...
      train_outcome_auc_score <- pROC::auc(outcome_true_y, outcome_pred_prob)
      message(paste("total=", total))
      ...
      message(sprintf("epoch %2d: train AE loss= %.4e, c acc= %.4e, outcome nll= %.4e, outcome_auc_score= %.4e, classifier loss= %.4e, outcome loss= %.4e, p_value loss= %.4e,",
      ...
      message(sprintf("        : test  AE loss= %.4e, c acc= %.4e, outcome nll= %.4e, outcome_auc_score= %.4e",
      ...
      message(paste("test_outcome_likelihood=",test_outcome_likelihood))
      message(paste("min_test_negloglikeli_record=",min_test_negloglikeli_record))
      message(test_outcome_likelihood <= min_test_negloglikeli_record)
      message(length(dict_p_value))
      message(dict_p_value)
      message(as.numeric(as.character(dict_p_value[length(dict_p_value)])))

      if ((test_outcome_likelihood <= min_test_negloglikeli_record) &
          as.numeric(as.character(dict_p_value[length(dict_p_value)])) <= 0.05) {
        message(paste("save model here! iter_i=", iter_i, ", epoch=", epoch))
  ...
  message(paste("number_reassign_list=", number_reassign_list))
  ...
  message(paste("random_state_list=", random_state_list))
  message(paste("saved_iter_list=", saved_iter_list))
  message(paste("saved_iter = ", saved_iter))
}
