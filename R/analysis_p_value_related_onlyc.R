...
#'   \item \code{data_v}: A matrix or data frame of additional covariates (not used in this function).
...
#'
#' @details
#' The \code{analysis_p_value_related_onlyc} function performs the following steps:
#' \enumerate{
...
analysis_p_value_related_onlyc <- function(data_train, num_clusters, if_check = FALSE) {
  ...
  message("dict_p_value only c=", unlist(dict_p_value), "")
  message("analysis done!")
}
